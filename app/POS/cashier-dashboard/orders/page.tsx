...
import { ChevronLeft, ChevronRight, Coffee, CreditCard, MoreHorizontal, Receipt, Search, UserRound, Utensils } from "lucide-react";
import { useEffect, useMemo, useState } from "react";

import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

type Order = { id: string; name: string; items: string; amount: number; time: string; channel: string; orderType: "Dine-in"; paymentMethod: "Cash" | "Card" | "GCash"; tableNumber: string; lineItems: { name: string; detail: string; quantity: number; price: number }[] };
...
const orders: Order[] = [
  { id: "#00482", name: "Mia Santos", items: "Iced latte, Matcha cloud", amount: 12.69, time: "Just now", channel: "Dine-in order", orderType: "Dine-in", paymentMethod: "Cash", tableNumber: "T04", lineItems: [{ name: "Iced latte", detail: "Regular · Oat milk", quantity: 1, price: 5.5 }, { name: "Matcha cloud", detail: "Large · Vanilla", quantity: 1, price: 6.25 }] },
  { id: "#00481", name: "Walk-in customer", items: "Blueberry cream", amount: 7.29, time: "3 min ago", channel: "Dine-in order", orderType: "Dine-in", paymentMethod: "GCash", tableNumber: "T03", lineItems: [{ name: "Blueberry cream", detail: "Regular · No changes", quantity: 1, price: 6.75 }] },
  { id: "#00480", name: "Noah Garcia", items: "Vanilla cold brew, Berry fizz", amount: 12.64, time: "6 min ago", channel: "Dine-in order", orderType: "Dine-in", paymentMethod: "Card", tableNumber: "T02", lineItems: [{ name: "Vanilla cold brew", detail: "Regular · Less ice", quantity: 1, price: 5.75 }, { name: "Berry fizz", detail: "Regular · No changes", quantity: 1, price: 5.95 }] },
  { id: "#00479", name: "Ava Cruz", items: "Citrus matcha", amount: 7.02, time: "12 min ago", channel: "Dine-in order", orderType: "Dine-in", paymentMethod: "Cash", tableNumber: "T05", lineItems: [{ name: "Citrus matcha", detail: "Large · No changes", quantity: 1, price: 6.5 }] },
  { id: "#00478", name: "Ethan Lee", items: "Hazelnut tonic", amount: 6.75, time: "18 min ago", channel: "Dine-in order", orderType: "Dine-in", paymentMethod: "Cash", tableNumber: "T01", lineItems: [{ name: "Hazelnut tonic", detail: "Regular · No changes", quantity: 1, price: 6.25 }] },
];
...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => orders.filter((order) => `${order.id} ${order.name} ${order.items}`.toLowerCase().includes(search.toLowerCase().trim())), [search]);
